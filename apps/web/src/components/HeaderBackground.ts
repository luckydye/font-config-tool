    ...
    const frame = (ms = 0) => {
      ...
      if (deltaTick > 1000 / 60) {
        if (this.ctxt) {
          ...
          for (let i = 0; i < this.renderableObjects.length; i += 1) {
            const text = this.renderableObjects[i];
            const attributes: string[] = [];

            for (let j = 0; j < text.uniforms.length; j += 1) {
              const attrib = text.uniforms[j];
              const { x } = text;
              const timeScaler = 800;
              const t = (1 + Math.sin((Date.now() / timeScaler) + (x * 0.5) + (j * 10))) / 2;
            ...
            this.ctxt.font = `${text.size}px Roboto Flex`;
