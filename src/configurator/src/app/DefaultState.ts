export default {
  content: {
    '923hf9239-f13f31f13f-dah97f3h2': {
      config: '39fhjfas-f082ufjoi32-das7912',
    },
    '8934jfg43-f3h7239fhj-d217hhjim': {
      config: '8943g934-f498jf3d42-dashi23',
    },
  },
  'font-configs': {
    '39fhjfas-f082ufjoi32-das7912': {
      title: 'Heading 1',
      font: {
        family: 'Roboto Flex',
        axes: [{
          tag: 'wght', min: 100, max: 900, defaultValue: 600,
        }, {
          tag: 'wdth', min: 20, max: 200, defaultValue: 50,
        }, {
          tag: 'slnt', min: 0, max: 100, defaultValue: 50,
        }, {
          tag: 'opsz', min: 0, max: 100, defaultValue: 50,
        }, {
          tag: 'YTUC', min: 0, max: 100, defaultValue: 50,
        }, {
          tag: 'YTLC', min: 0, max: 100, defaultValue: 50,
        }, {
          tag: 'YTFI', min: 0, max: 100, defaultValue: 50,
        }, {
          tag: 'YTDE', min: 0, max: 100, defaultValue: 50,
        }, {
          tag: 'YTAS', min: 0, max: 100, defaultValue: 50,
        }, {
          tag: 'YOPQ', min: 0, max: 100, defaultValue: 50,
        }, {
          tag: 'XTRA', min: 0, max: 100, defaultValue: 50,
        }, {
          tag: 'XOPQ', min: 0, max: 100, defaultValue: 50,
        }, {
          tag: 'GRAD', min: 0, max: 100, defaultValue: 50,
        }],
        files: { regular: '../fonts/RobotoFlex[GRAD,XOPQ,XTRA,YOPQ,YTAS,YTDE,YTFI,YTLC,YTUC,opsz,slnt,wdth,wght].ttf' },
        creators: ['Google'],
        linkUrl: '/fonts/RobotoFlex.css',
      },
      'axes-wght': 700,
      'font-size': 32,
    },
    '8943g934-f498jf3d42-dashi23': {
      title: 'Paragraph',
      font: {
        family: 'Open Sans',
        axes: [{
          tag: 'wdth', min: 75, max: 100, defaultValue: 100,
        }, {
          tag: 'wght', min: 300, max: 800, defaultValue: 400,
        }],
        creators: ['Steve Matteson'],
        files: {
          300: 'http://fonts.gstatic.com/s/opensans/v27/memSYaGs126MiZpBA-UvWbX2vVnXBbObj2OVZyOOSr4dVJWUgsiH0C4nY1M2xLER.ttf', 500: 'http://fonts.gstatic.com/s/opensans/v27/memSYaGs126MiZpBA-UvWbX2vVnXBbObj2OVZyOOSr4dVJWUgsjr0C4nY1M2xLER.ttf', 600: 'http://fonts.gstatic.com/s/opensans/v27/memSYaGs126MiZpBA-UvWbX2vVnXBbObj2OVZyOOSr4dVJWUgsgH1y4nY1M2xLER.ttf', 700: 'http://fonts.gstatic.com/s/opensans/v27/memSYaGs126MiZpBA-UvWbX2vVnXBbObj2OVZyOOSr4dVJWUgsg-1y4nY1M2xLER.ttf', 800: 'http://fonts.gstatic.com/s/opensans/v27/memSYaGs126MiZpBA-UvWbX2vVnXBbObj2OVZyOOSr4dVJWUgshZ1y4nY1M2xLER.ttf', regular: 'http://fonts.gstatic.com/s/opensans/v27/memSYaGs126MiZpBA-UvWbX2vVnXBbObj2OVZyOOSr4dVJWUgsjZ0C4nY1M2xLER.ttf', '300italic': 'http://fonts.gstatic.com/s/opensans/v27/memQYaGs126MiZpBA-UFUIcVXSCEkx2cmqvXlWq8tWZ0Pw86hd0Rk5hkaVcUwaERZjA.ttf', italic: 'http://fonts.gstatic.com/s/opensans/v27/memQYaGs126MiZpBA-UFUIcVXSCEkx2cmqvXlWq8tWZ0Pw86hd0Rk8ZkaVcUwaERZjA.ttf', '500italic': 'http://fonts.gstatic.com/s/opensans/v27/memQYaGs126MiZpBA-UFUIcVXSCEkx2cmqvXlWq8tWZ0Pw86hd0Rk_RkaVcUwaERZjA.ttf', '600italic': 'http://fonts.gstatic.com/s/opensans/v27/memQYaGs126MiZpBA-UFUIcVXSCEkx2cmqvXlWq8tWZ0Pw86hd0RkxhjaVcUwaERZjA.ttf', '700italic': 'http://fonts.gstatic.com/s/opensans/v27/memQYaGs126MiZpBA-UFUIcVXSCEkx2cmqvXlWq8tWZ0Pw86hd0RkyFjaVcUwaERZjA.ttf', '800italic': 'http://fonts.gstatic.com/s/opensans/v27/memQYaGs126MiZpBA-UFUIcVXSCEkx2cmqvXlWq8tWZ0Pw86hd0Rk0ZjaVcUwaERZjA.ttf',
        },
        linkUrl: 'https://fonts.googleapis.com/css2?family=Open+Sans:wdth,wght@75..100,300..800&display=swap',
      },
      'axes-wght': 445,
      'axes-wdth': 87.75,
      'font-size': 16,
    },
  },
};
